...
- Scenarios = пользовательские действия (явные и неявные), начинаются с глагола.
- Включай implicit действия (автозаполнение, фильтрация, зависимые состояния), если система реагирует на них.
- Разбивай сложные условия (“если/при/когда…”) на последовательные варианты действий и outcomes.
- Покрывай ВСЕ ветвления UX, если результат отличается (даже если описано косвенно).
- Не дроби Story ради техники, но внутри Story добавляй все пользовательские ветки.
- 🚨 Scenario = действие пользователя, НЕ поведение системы!
- ❌ Запрещено описывать Scenario через “Отобразить”, “Появляется” — это Codes.
...
- Code должен описывать ВСЕ реакции системы на действия пользователя — включая любые UX outcomes, валидации, автоматические состояния, алерты, блокировки, автозаполнение и бизнесовые ветвления, если они описаны в требованиях.
- Не создавай отдельные негативные Story или Scenario, если негативный/ошибочный кейс не описан явно — любые негативные ветвления оформляй как отдельный Scenario/Code внутри Story.
...
- Чем проще структура модели — тем лучше. Не раздробляй модель на лишние Story, но обязательно покрывай все пользовательские ветвления внутри Story.

🚨 КРИТИЧНО: СТРОГО СЛЕДУЙ ТРЕБОВАНИЯМ! НЕ ВЫДУМЫВАЙ НЕГАТИВНЫЕ СЦЕНАРИИ!

- ❌ ЗАПРЕЩЕНО создавать codes с ошибками (500, 400, 404, таймауты), если они НЕ упомянуты в требованиях!
- ❌ ЗАПРЕЩЕНО создавать codes с "пустой ответ", "ошибка сервера", "превышено время ожидания", если это НЕ описано в требованиях!
- ❌ ЗАПРЕЩЕНО добавлять негативные codes (ошибки, таймауты) в позитивные scenarios!
...
- 🎯 ПРАВИЛО: Code = ТОЛЬКО то, что явно описано в требованиях! НЕ выдумывай негативные реакции системы!
