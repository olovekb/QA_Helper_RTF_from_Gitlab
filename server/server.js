...
2. **Жизненный цикл (States):** Если есть статусы (Активна/Неактивна), найди сценарии смены статуса (Активация, Деактивация).
   - Если есть требования с периодическим опросом статуса (повторные запросы каждые N секунд, статусы NTST/RCVD/ACWP/RJCT), создай отдельные Scenario для: ожидания (polling), успешной оплаты, отказа, неожиданных/пустых ответов. Не сваливай всё в один Scenario.

3. **Точки входа:** Проверь все места: Меню, Вкладки, Модальные окна, Списки.
   - Если в требованиях перечислены отдельные инструменты, режимы или варианты действий в одном экране (например, "обрезка", "поворот 90/180/270", "фильтры: чёрно-белый, сепия, винтаж"), НЕ объединяй их в один Scenario.
   - Создай по крайней мере по одному Scenario на КАЖДУЮ ГРУППУ инструментов:
     - отдельный Scenario для базового использования инструмента/режима (открытие/вход)
     - отдельный Scenario для каждого типа инструмента или группы похожих инструментов (например, все типы фильтров можно сгруппировать в один Scenario, но обрезка и поворот — отдельные)

4. **Валидация:** Найди ограничения полей (макс. длина, обязательность). Создай сценарии ошибок валидации.
   - Если в требованиях описано, что действие или кнопка доступны ТОЛЬКО при выполнении условия (например, "кнопка доступна при наличии изображения" или "кнопка активна после заполнения формы"), ОБЯЗАТЕЛЬНО создай:
     - Scenario на ДОСТУПНОЕ состояние (условие выполнено, кнопка активна)
     - Scenario на НЕДОСТУПНОЕ состояние (условие не выполнено, кнопка отключена или недоступна)
   - В недоступном сценарии:
     - Code.type = "frontend"
     - code.text описывает именно состояние интерфейса: "Кнопка ... отображается в неактивном состоянии / недоступна для нажатия", без отправки запросов.

5. **API Binding:** Если в тексте упомянут API метод (даже в конце документа), он ОБЯЗАН быть использован в codes.
   - Для каждого явно описанного API-метода:
     - Всегда создай хотя бы один Scenario с успешным ответом (200 OK) с полным набором полей из требований.
     - Если в требованиях явно указан хотя бы один тип ошибки (например, 400, 404, 500 или текст ошибки), создай ОТДЕЛЬНЫЙ негативный Scenario для этой ошибки.
...
- Story = пользовательская цель с бизнес-правилами (НЕ контролы, НЕ "Загрузка...", НЕ по разделам документа!). Определяй Story по бизнес-ценности для пользователя, а НЕ по структуре документа.
  - Для одного бизнес-процесса (например, выпуск функциональной ссылки) создай несколько Story по подцелям пользователя:
    - Вход в сценарий (через кнопки/меню).
    - Заполнение и валидация формы.
    - Выпуск/сохранение.
    - Просмотр результата и статусы.
- Scenario = одно простое пользовательское действие (НЕ техническое, НЕ процесс!). Текст начинается с глагола действия пользователя: "Нажать", "Ввести", "Выбрать", "Настроить", "Создать", "Сохранить", "Отменить". ❌ НЕ "Загрузить страницу", "Открыть приложение" - это технические действия! ❌ НЕ "Выполнить пользовательское действие" - это placeholder! ❌ НЕ "Настроить параметры... и создать..." - это процесс, разбей на отдельные Scenario!
  - Если в одном требовании описано несколько действий пользователя (нажать → ввести → выбрать → отправить), разбей их на несколько Scenario, по одному действию в каждом.
- Code = полная реакция системы на действие. Каждая реакция отдельным объектом, строго type: frontend | backend.
  - frontend = поведение фронтенда (отправка запросов, отображение UI, реакции интерфейса)
  - backend = поведение бэкенда (возврат ответов, обработка запросов)
  - ❌ НЕ используй в Code: "Обнаружить", "Проверить", "Получить ошибку", "Переключиться" - это действия/проверки, не реакции системы!

🔄 АЛГОРИТМ: ЧИТАЙ ТРЕБОВАНИЯ → ПОНИМАЙ БИЗНЕС-ЦЕННОСТЬ → ВЫДЕЛЯЙ ДЕЙСТВИЯ → ОПИСЫВАЙ РЕАКЦИИ

ШАГ 1: АНАЛИЗ ТРЕБОВАНИЙ (НЕ СТРУКТУРЫ ДОКУМЕНТА!)
...
- ✅ Объединяй связанные требования в одну Story по бизнес-смыслу только если они описывают ОДНУ и ту же бизнес-цель без разных стадий процесса. Если есть разные стадии (вход, заполнение, выпуск, просмотр, статусы) — дели на разные Story.
- Пример: Если в разделе 3.1 описана проверка подключения СБП, а в 3.2 - создание QR-кода, это ОДНА Story "Создание QR-кода для физических лиц", а НЕ две отдельные Story!

ШАГ 2: ВЫДЕЛЕНИЕ ДЕЙСТВИЙ ПОЛЬЗОВАТЕЛЯ
- Найди все явные и неявные действия пользователя (НЕ технические действия типа "Загрузить страницу"!).
- Для каждого действия пользователя создай Scenario (глагол действия: "Нажать", "Ввести", "Выбрать").
- Действие = то, что пользователь делает для достижения бизнес-цели
- ❌ НЕ сливай несколько действий в один Scenario! Если в одном требовании описано несколько действий пользователя (нажать → ввести → выбрать → отправить), разбей их на несколько Scenario, по одному действию в каждом.

ШАГ 3: ОПИСАНИЕ РЕАКЦИЙ СИСТЕМЫ
- Для Scenario распиши цепочку Codes: frontend (отправка запросов) → backend (возврат ответов) → frontend (отображение UI), пока реакция не завершена.
- Code = реакция системы на действие пользователя, НЕ действие пользователя!

ШАГ 4: ПРОВЕРКА
- Не смешивай действия и реакции. Не используй placeholder-формулировки.
- Story описывает бизнес-ценность, а НЕ техническую операцию или раздел документа!

📌 ЖЁСТКИЕ ПРАВИЛА

**КАК АНАЛИЗИРОВАТЬ ТРЕБОВАНИЯ:**
1. ❌ НЕ смотри на структуру документа (разделы 3.1, 3.2, 3.3...) и НЕ создавай Story по каждому разделу!
2. ✅ Читай текст требований и понимай бизнес-ценность: "Что хочет получить пользователь?"
3. ✅ Объединяй связанные требования в одну Story только если они описывают ОДНУ и ту же бизнес-цель без разных стадий процесса. Если есть разные стадии (вход, заполнение, выпуск, просмотр, статусы) — дели на разные Story.
4. ✅ Анализируй действия пользователя и реакции системы, а НЕ структуру документа

**STORY:**
- ❌ НЕ создавай Story "Загрузка страницы...", "Переключение между вкладками..." - это технические операции!
- ❌ НЕ создавай Story по разделам документа (3.1, 3.2...) - это техническая структура!
- ✅ Создавай Story по бизнес-ценности: "Создание QR-кода для физических лиц", "Выставление QR-кода"
- ✅ Story = ответ на вопрос "Что хочет получить пользователь?", а НЕ "Какой раздел документа?"
- ✅ Для одного бизнес-процесса создавай несколько Story по подцелям: вход, заполнение формы, выпуск/сохранение, просмотр результата и статусы

**ОБЩИЕ:**
- Не выдумывай API/ошибки/негативы, если их нет в тексте.
- Не создавай Story/Scenario ради оглавления или технического раздела. Есть ценность → есть Story; нет ценности → пропусти.
- Scenario = действие пользователя (НЕ техническое!). ❌ "Загрузить страницу", "Открыть приложение" - это НЕ действия пользователя! ❌ "Выполнить пользовательское действие" - это placeholder! ✅ "Нажать кнопку", "Ввести текст" - это действия пользователя.
- ❌ НЕ сливай несколько действий в один Scenario! Если в одном требовании описано несколько действий пользователя (нажать → ввести → выбрать → отправить), разбей их на несколько Scenario, по одному действию в каждом.
- Не дели одно и то же действие на несколько Scenario только из-за разных мест в UI — различия местом перенесём в тест-кейсы через параметризацию.
- Не оставляй пустых codes. Нет реакции → нет Scenario.
- Не пиши префиксов "API:", "UI:" в code.text.
- Code.type = ТОЛЬКО "frontend" или "backend". "Отправляется GET..." = frontend (фронтенд отправляет запрос), "Возвращается 200 OK..." = backend (бэкенд возвращает ответ).
- Code = ТОЛЬКО реакции системы! ❌ НЕ используй: "Обнаружить непустой sbpId", "Получить ошибку", "Переключиться на вкладку" - это действия/проверки! ✅ Используй: "Отправляется GET...", "Возвращается 200 OK...", "Отображается страница..."
- IDs уникальные: Feature "f...", Story "s...", Scenario "sc...", Code "c..." (либо uuid).

**ДОСТУПНОСТЬ ДЕЙСТВИЙ:**
- Если в требованиях описано условие доступности кнопки/действия (например, "кнопка доступна при наличии изображения", "кнопка активна после заполнения формы"), создай ДВА Scenario:
  - Scenario для ДОСТУПНОГО состояния (условие выполнено, действие доступно)
  - Scenario для НЕДОСТУПНОГО состояния (условие не выполнено, действие недоступно)
- В Scenario недоступного состояния:
  - Code.type = "frontend"
  - code.text описывает состояние UI: "Кнопка ... отображается в неактивном состоянии / недоступна для нажатия"
  - НЕ добавляй отправку запросов в недоступном состоянии

**ИНСТРУМЕНТЫ И РЕЖИМЫ:**
- Если в требованиях перечислены отдельные инструменты, режимы или варианты действий в одном экране (например, "обрезка", "поворот", "фильтры"), НЕ объединяй их в один Scenario.
- Создай отдельные Scenario для каждой группы инструментов:
  - Базовый Scenario для входа/открытия инструмента
  - Отдельный Scenario для каждого типа инструмента или группы похожих (например, все типы фильтров можно сгруппировать, но обрезка и поворот — отдельные)
- Это правило применяется к любым наборам опций: тарифы, роли, фильтры, виды сортировки и т.д.

**API-МЕТОДЫ И ОШИБКИ:**
- Для каждого явно описанного API-метода всегда создай хотя бы один Scenario с успешным ответом (200 OK) с полным набором полей из требований.
- Негативные Scenario создавай ТОЛЬКО если в требованиях явно указан хотя бы один тип ошибки (400, 404, 500 или текст ошибки).
...
function buildRefineModelSystemPrompt() {
    return `
Ты — Редактор тестовой модели. Твоя задача — доработать существующую модель на основе замечаний ревьюера, НЕ ломая уже корректные части.

🎯 ГЛАВНАЯ ЦЕЛЬ:
Исправить указанные проблемы, сохранив все корректные части модели без изменений.

📋 КОНТРАКТ ИЗМЕНЕНИЙ:

**РАЗРЕШЕНО:**
- ✅ Добавлять новые Story/Scenario/Code для исправления проблем
- ✅ Править текст существующих элементов (если указано в замечаниях)
- ✅ Разбивать слишком "толстые" Story на несколько более мелких
- ✅ Добавлять недостающие сценарии (статусы, валидация, ошибки)
- ✅ Удалять явно указанные дубли или некорректные элементы

**ЗАПРЕЩЕНО:**
- ❌ Выкидывать произвольные части модели без указания причины в замечаниях
- ❌ Удалять Feature без явного указания в замечаниях
- ❌ Менять ID существующих элементов (кроме случаев явного указания)
- ❌ Переписывать корректные части, которые не упомянуты в замечаниях

🔍 АЛГОРИТМ РАБОТЫ:

1. **Анализ замечаний:** Определи, какие именно части модели нужно изменить
2. **Ограничение радиуса:** Изменяй ТОЛЬКО те части, которые явно указаны в issues/замечаниях
3. **Сохранение корректного:** Все остальные части должны остаться семантически неизменными
4. **Исправление проблем:** Примени исправления строго по замечаниям

📌 ПРАВИЛА СТРУКТУРЫ (как в buildModelSystemPrompt):
- Feature = бизнес-домен/ценность системы
- Story = пользовательская цель с бизнес-правилами (НЕ технические операции!)
- Scenario = одно простое пользовательское действие
- Code = реакция системы (frontend | backend)

⚠️ ВАЖНО:
- Если в замечаниях указано "слишком толстые Story" — разбей их на несколько Story по подцелям
- Если указано "нет сценариев по статусам" — добавь отдельные Scenario для polling, успешной оплаты, отказа
- Если указано "убрать дубли Code" — удали дублирующиеся Code, оставив один
- Сохраняй все ID существующих элементов, которые не нужно менять

📦 ВЫХОД:
- Полный обновлённый JSON-массив модели в том же формате
- Все существующие ID должны быть сохранены (кроме явно удалённых элементов)
- Новые элементы получают новые UUID
`.trim();
}
...
function buildModelUserPrompt({
    ...
    // В режиме без чанков (один документ) упрощаем контекст, чтобы модель не пыталась "дописывать" в существующие Story
    const contextInfo = totalChunks > 1 && previousContext && Array.isArray(previousContext) && previousContext.length > 0
        ? (() => {
...
${totalChunks === 1 ? `Для текущего документа ожидается:
- ОДНА Feature: "Эквайринг: выпуск функциональных QR-ссылок" (или аналогичная по смыслу).
- 4–5 Story по подцелям: вход, проверка подключения и загрузка страницы, заполнение формы, выпуск/сохранение, страница с выпущенным QR и статусы.` : ''}
...
- **Layer:** "Integration backend Tests"
- **Tags:** ["S"] (Всегда!)
- Для каждого уникального сочетания HTTP-метода, URL и кода ответа из модели (200/400/404/500 и т.п.) должен быть минимум один тест.
- Если один и тот же URL в модели имеет несколько кодов ответа, создай отдельные тесты для каждого кода.
` : ''}
...
- ❌ Step: "Открыть страницу X" как подготовка (если цель теста не в этом). Всё, что нужно сделать ДО начала проверки (авторизация, навигация, открытие модалки), пиши в Precondition.
  ✅ ИСКЛЮЧЕНИЕ: Если сам тест проверяет отображение страницы/модалки по умолчанию, тогда шаг "Открыть страницу/модалку" допускается как основное действие.
