...
${storiesList}`;
    ...
    return `
📋 ЗАДАНИЕ: Создай тестовую модель Feature → Story → Scenario → Code на основе ЗАДАННОЙ структуры.

🚨 КРИТИЧНО: Features и Stories УЖЕ ОПРЕДЕЛЕНЫ! НЕ создавай новые!
...
═══════════════════════════════════════════════════════════════
📚 ЭТАЛОННАЯ МОДЕЛЬ (СТРОГО СЛЕДУЙ ЭТОМУ ФОРМАТУ!)
═══════════════════════════════════════════════════════════════
\`\`\`json
${formattedExample}
\`\`\`

🔑 КЛЮЧЕВЫЕ ПРАВИЛА ИЗ ПРИМЕРА:
1. Story = бизнес-ценность для пользователя (НЕ техническая реализация! НЕ "Загрузка страницы", "Обработка кнопки")
   ✅ "Регистрация в ББО", "Создание QR-кода для физического лица"
   ❌ "Загрузка страницы", "Проверка подключения", "Обработка ошибок", "Загрузка страницы выставления QR-кода"
   🚨 КРИТИЧНО: Если в требованиях есть раздел "Загрузка страницы X" - это НЕ отдельная Story! Это часть основной бизнес-ценности!
   🚨 Вся логика загрузки, переключения вкладок, обработки ошибок на одной странице = ОДНА Story!
2. Scenario = атомарное действие пользователя, группируй по РЕАКЦИИ СИСТЕМЫ, НЕ по месту размещения!
   ✅ ОДИН Scenario: "Нажать на кнопку 'Создать QR-код'" (если везде одинаковая реакция)
   ❌ ТРИ отдельных Scenario: для модального окна, для страницы, для вкладки (если реакция одинаковая!)
   🚨 Правило: Различия в местах (модальное окно, страница, вкладка) = параметризация в тест-кейсах, НЕ в модели!
   🚨 Создавай ОТДЕЛЬНЫЕ Scenarios ТОЛЬКО если РЕАКЦИЯ СИСТЕМЫ РАЗНАЯ (разные API, разные страницы, разные условия)
3. Codes = полная цепочка реакций системы (frontend → backend → frontend → integration) с обязательным type
   🚨 КРИТИЧНО: 
   - "Отправляется GET **/api/...**" → type: "frontend" (НЕ "backend"!)
   - "Возвращается 200 OK..." → type: "backend"
   - Codes НЕ могут содержать элементы с id, начинающимися с "sc" (это Scenarios, а не Codes!)
   - Integration codes = системные реакции ("Сравнивается X с Y"), НЕ действия пользователя ("Переключиться", "Нажать")

═══════════════════════════════════════════════════════════════
📖 ПРАВИЛА ДЛЯ SCENARIOS
═══════════════════════════════════════════════════════════════

✅ Scenario = атомарное действие пользователя (глагол: "Нажать", "Ввести", "Выбрать")
✅ Для каждой Story создавай Scenarios на основе явных действий из требований
✅ Если в требованиях есть условия ("если...", "при...") — создавай отдельный Scenario для каждой ветки
✅ Каждая Story ДОЛЖНА содержать ≥1 Scenario (пустые Stories запрещены!)

🚨 КРИТИЧНО: ПАРАМЕТРИЗАЦИЯ vs РАЗНЫЕ SCENARIOS
- Если одно и то же действие в разных местах (модальное окно, страница, вкладка), но РЕАКЦИЯ СИСТЕМЫ ОДИНАКОВАЯ → ОДИН Scenario!
- Различия в местах размещения кнопки/элемента = параметризация в тест-кейсах (не в модели!)
- Пример: кнопка "Создать QR-код" в модальном окне "Создать платёж" ИЛИ в модальном окне "+ Платёж" ИЛИ на странице "Эквайринг" → если везде отправляется GET **/api/...** с одинаковой реакцией → ОДИН Scenario "Нажать на кнопку 'Создать QR-код'"
- Создавай ОТДЕЛЬНЫЕ Scenarios ТОЛЬКО если РЕАКЦИЯ СИСТЕМЫ РАЗНАЯ (разные API, разные страницы, разные условия)

❌ ЗАПРЕЩЕНО:
- "Проверить", "Убедиться" — это НЕ действия пользователя!
- "Отобразить", "Появляется" — это поведение системы (Code), НЕ Scenario!
- "Дождаться", "Попытаться" — убрать!
- Служебные названия: "Выполнить пользовательское действие", "Сценарий 1"
- Создавать отдельные Scenarios для одного действия в разных местах, если реакция системы одинаковая

═══════════════════════════════════════════════════════════════
⚙️ ПРАВИЛА ДЛЯ CODES
═══════════════════════════════════════════════════════════════

✅ Code = ОДНА реакция системы (НЕ объединяй несколько!)
✅ Для каждого Scenario формируй ПОЛНУЮ цепочку: frontend → backend → frontend → integration

ТИПЫ (КРИТИЧНО ПРАВИЛЬНО ОПРЕДЕЛЯЙ!):
- **frontend**: "Отправляется GET **/api/...**", "Отображается модальное окно", "Отображается ошибка", "Осуществляется переход"
  🚨 "Отправляется" = ВСЕГДА type: "frontend" (НЕ "backend"!)
- **backend**: "Возвращается 200 OK с {id, name}", "Возвращается 400 Bad Request с {error: '...'}" (только если описано!)
  🚨 "Возвращается" = ВСЕГДА type: "backend"
- **integration**: "Сравнивается X с Y", "Создаётся запись в БД", "Проверяется условие: X равен Y"
  🚨 Integration = системные реакции, проверки, сравнения (НЕ действия пользователя!)

❌ ЗАПРЕЩЕНО:
- Префиксы "API:", "UI:", "Frontend:", "Backend:" в text
- Объединение нескольких реакций в один code
- Выдумывать ошибки/негативные реакции, если их НЕТ в требованиях!
- Code = ТОЛЬКО то, что явно описано в требованиях!
- В codes элементы с id, начинающимися с "sc" (это Scenarios, а не Codes!)
- Integration codes с действиями пользователя ("Переключиться", "Нажать", "Выбрать") - это НЕ системные реакции!

═══════════════════════════════════════════════════════════════
🔍 АЛГОРИТМ
═══════════════════════════════════════════════════════════════

1. Найди все явные действия пользователя → группируй по РЕАКЦИИ СИСТЕМЫ
   - Если одно действие в разных местах, но реакция одинаковая → ОДИН Scenario
   - Если реакция разная → ОТДЕЛЬНЫЕ Scenarios
2. Найди все условия ("если", "при") → создай отдельный Scenario для каждой ветки
3. Для каждого Scenario определи все реакции системы → создай codes (frontend → backend → frontend → integration)
4. Проверь: каждая Story имеет ≥1 Scenario с полной цепочкой codes
    ...
    "stories": [
      {
        ...
        "scenarios": [
          {
            "id": "uuid",
            "text": "Действие пользователя (явное или implicit)",
            "codes": [
              {"id": "uuid", "text": "Реакция системы 1", "type": "frontend"},
              {"id": "uuid", "text": "Реакция системы 2", "type": "backend"},
              {"id": "uuid", "text": "Реакция системы 3", "type": "frontend"},
              {"id": "uuid", "text": "Реакция системы 4", "type": "integration"}
            ]
          }
        ]
      }
...
function validateTestModel(model, reqStructure) {
    ...
    for (let fIdx = 0; fIdx < reqStructure.features.length && fIdx < model.length; fIdx++) {
        ...
        const technicalStoryKeywords = ['загрузка', 'проверка', 'обработка', 'реализация', 'обработка ошибок'];
        for (const story of generatedFeature.stories || []) {
            ...
            for (const keyword of technicalStoryKeywords) {
                if (storyText.includes(keyword) && !storyText.includes('создание') && !storyText.includes('регистрация') && !storyText.includes('просмотр')) {
                    report.errors.push(`Story "${story.text}" содержит технический термин "${keyword}" - это не бизнес-ценность!`);
                    report.valid = false;
                }
            }
        }
        ...
        for (const story of generatedFeature.stories || []) {
            for (const scenario of story.scenarios || []) {
                ...
                for (const code of scenario.codes || []) {
                    ...
                    // Проверка integration codes - не должны содержать действия пользователя
                    if (code.type === 'integration') {
                        const userActions = ['нажать', 'переключиться', 'выбрать', 'ввести', 'кликнуть', 'открыть'];
                        for (const action of userActions) {
                            if (codeText.includes(action)) {
                                report.errors.push(`Integration code "${code.text}" содержит действие пользователя "${action}" - это неправильно! Integration = системные реакции, не действия!`);
                                report.valid = false;
                            }
                        }
                    }
...
`
        ...
        const SYSTEM_PROMPT = `
Ты — Системный Архитектор и QA Lead, генерирующий универсальную тестовую модель на основе requirements для любых бизнес-документов (финтех, портал, ecommerce, корпоративные системы и т.д.).

═══════════════════════════════════════════════════════════════
🏗️ DOMAIN DRIVEN TESTING (DDT) - СТРУКТУРА ТЕСТОВОЙ МОДЕЛИ
═══════════════════════════════════════════════════════════════

МЫСЛИ КАК DOMAIN EXPERT: Модель описывает не просто текст требований, а ПОВЕДЕНИЕ СИСТЕМЫ во всех его проявлениях, включая implicit (неявные) ветвления и альтернативные флоу.

Feature → Story → Scenario → Code

**Feature** = ДОМЕН / ЭПИК. Крупная функциональная область системы.
  Примеры: домен платежных операций, домен электронного документооборота, домен управления пользователями, домен каталога товаров и т.д.
  Feature объединяет связанные пользовательские истории в одну функциональную область.
...
**Story** = USER STORY / CAPABILITY. Потребность пользователя с бизнес-правилами.
  ...
  Story описывает ЧТО хочет получить пользователь, а НЕ техническую реализацию.
  
  ✅ Правильно: "Регистрация пользователя", "Создание заказа", "Просмотр каталога", "Оплата товара", "Управление профилем"
  ❌ Неправильно: "Реализация кнопки X", "API метод получения данных", "Чек-бокс Y", "Вкладка Z"
  
  🚨 ПРАВИЛО: Story = ценность для пользователя, НЕ техническая реализация!
  ✔️ Не описывай Story через технические термины — только через пользовательские цели.

**Scenario** = USER INTERACTION / TRIGGER. Атомарное воздействие на систему (одно действие, включая implicit).
  🚨 КРИТИЧНО: Scenario = АТОМАРНОЕ действие (один клик/ввод/выбор), но учитывай и неявные (implicit) пользовательские триггеры.
  Scenario описывает ОДНО пользовательское воздействие (явное или неявное), которое триггерит реакцию системы.
  
  🎯 УНИВЕРСАЛЬНЫЙ ПРИНЦИП: Для каждой Story выявляй и создавай отдельные Scenarios НЕ ТОЛЬКО на основании явно сформулированных действий, но и на основе ВСЕХ возможных логических и бизнес-вариантов поведения пользователя, которые подразумеваются логикой требований.
  
  ✅ ПРАВИЛЬНЫЕ ФОРМУЛИРОВКИ (универсальные примеры):
  - "Нажать на кнопку [название]"
  - "Выбрать значение из выпадающего списка"
  - "Ввести данные в поле ввода"
  - "Кликнуть на элемент из списка"
  - "Выбрать опцию из меню"
  - "Загрузить форму с предзаполненными данными" (implicit триггер)
  - "Изменить параметр фильтрации" (implicit триггер)
  - "Убрать фокус с поля ввода" (implicit триггер)
  
  ❌ НЕПРАВИЛЬНО - множественные действия в одном Scenario:
  - "Открыть окно, выбрать статус и отфильтровать список" → Раздели на 3 Scenario!
  - "Нажать 'Создать', заполнить форму и получить ошибку" → Раздели на 2 Scenario!
  - "Выбрать товар, добавить в корзину и отобразить уведомление" → Раздели на 3 Scenario!
  
  ❌ НЕПРАВИЛЬНО - нет действия пользователя:
  - "Проверить отсутствие результатов" → Нет действия пользователя!
  - "Проверить превышение лимита" → Нет действия пользователя!
  - "Применить скидку X%" → Нет UI-действия! → ✅ "Нажать кнопку 'Применить скидку'"
  
  🚨 ЗАПРЕЩЁННЫЕ ГЛАГОЛЫ в начале Scenario:
  ❌ "Проверить", "Убедиться" - это НЕ действия пользователя!
  ❌ "Попытаться" - НЕ используй! Просто опиши действие
  ...
  Применить, Повернуть, Подтвердить, Закрыть, Сохранить, Отменить, Изменить
  
  🚨 КРИТИЧНО: Scenario = действие пользователя (явное или implicit trigger), НЕ поведение системы!
  ❌ "Отобразить кнопку X при условии Y" - НЕПРАВИЛЬНО! Это поведение системы!
  ✅ Правильно: "Выполнить действие, которое триггерит условие Y" (действие пользователя)
     А "Отобразить кнопку..." - это Code (поведение системы)!
  
  🎯 ВЫЯВЛЕНИЕ IMPLICIT (НЕЯВНЫХ) СЦЕНАРИЕВ:
  ▸ Анализируй требования на предмет скрытых пользовательских триггеров
  ▸ Если система реагирует на изменение данных, состояния, условий — выделяй это как отдельный Scenario
  ▸ Примеры implicit триггеров: автозаполнение при загрузке, фильтрация при изменении параметров, валидация при потере фокуса, автоматическое обновление UI, блокировка/разблокировка элементов
  ▸ Разбивай неявные требования на действия пользователя и реакции системы
  ▸ Если что-то выполняется "при/после/если/когда ...", трактуй это как отдельный Scenario и Code
  ▸ При автоматической, условной или ветвящейся логике — добавляй все возможные пользовательские варианты и реакции системы на них
  ▸ В бизнес-ветках (валидности, алерты, автозаполнения), даже если действие описано через "появляется", "присваивается", "отображается", выделяй скрытый пользовательский trigger как Scenario
  
  🎯 ВЫЯВЛЕНИЕ ВСЕХ ВОЗМОЖНЫХ ВАРИАНТОВ:
  ▸ Для каждого действия анализируй ВСЕ возможные варианты: успех, неуспех (если описан), отмена, отсутствие данных, невалидные параметры (если описана валидация), альтернативные действия, возврат, корректировка, автоматические UI-события
  ▸ Каждый вариант = отдельный Scenario, если результат отличается
  ▸ Для каждой Story обязательно выделяй ВСЕ ветвления действий пользователя (и позитивные, и негативные, если явно описаны). Не пропускай варианты, даже если они описаны косвенно
...
**Code** = SYSTEM CONTRACT / REACTION. Контракт поведения системы на Scenario.
  ...
  **Data_State (White Box, type: "integration"):**
  ...
  🎯 АЛГОРИТМ "ACTION → REACTION":
    ...
    - Code (Reaction) = цепочка реакций системы (Frontend запрос → Backend ответ → Frontend отображение → Integration действия)
    - Для каждого Scenario формируй ПОЛНУЮ цепочку всех реакций системы
  
  ✅ ПРАВИЛЬНО (Frontend - отправка запросов):
  - "Отправляется GET **/api/v1/[resource]**" + type: "frontend"
  - "Отправляется POST **/api/v1/[resource]** с параметрами param1 (type1), param2 (type2)" + type: "frontend"
  - "Отправляется PUT **/api/v1/[resource]/{id}** с параметрами param1 (type1)" + type: "frontend"
  - "Отправляется DELETE **/api/v1/[resource]/{id}**" + type: "frontend"
  
  ✅ ПРАВИЛЬНО (Backend - ответы с JSON):
  - "Возвращается 200 OK с массивом: [{field1, field2, field3}]" + type: "backend"
  - "Возвращается 200 OK с {field1, field2, field3}" + type: "backend"
  - "Возвращается 400 Bad Request с {error: 'описание ошибки'}" + type: "backend" (только если описано в требованиях!)
  - "Возвращается 404 Not Found с {error: 'описание ошибки'}" + type: "backend" (только если описано в требованиях!)
  
  ✅ ПРАВИЛЬНО (Frontend - отображение UI):
  - "Отображается модальное окно '[название]' с полями [описание]" + type: "frontend"
  - "Отображается список элементов с [описание полей]" + type: "frontend"
  - "Отображается ошибка '[текст ошибки]'" + type: "frontend"
  - "Скрывается форма ввода" + type: "frontend"
  - "Поле становится недоступным для редактирования" + type: "frontend"
  - "Заполняется список из параметра X" + type: "frontend"
  
  ✅ ПРАВИЛЬНО (Integration - интеграционные действия):
  - "Сравнивается значение X с Y" + type: "integration"
  - "Проверяется условие: X равен Y" + type: "integration"
  - "Создаётся запись в БД с параметрами X, Y" + type: "integration"
  - "Обновляется статус в кэше" + type: "integration"
  - "Отправляется запрос во внешнюю систему Z" + type: "integration"
  ...
  ▸ Code должен описывать ВСЕ реакции системы на действия пользователя — включая любые UX outcomes, валидации, автоматические состояния, алерты, блокировки, автозаполнение и бизнесовые ветвления, если они описаны в requirements.
  ▸ Не создавай отдельные негативные Story или Scenario без явного упоминания в требованиях — негативные ветвления оформляй как отдельные Scenario/Code внутри той же Story.
  
  🎯 СТРУКТУРА CODE ДЛЯ ОДНОГО SCENARIO (ПОЛНАЯ ЦЕПОЧКА):
  Для каждого Scenario формируй ПОЛНУЮ цепочку всех реакций системы:
  
  1. Frontend отправляет запрос: "Отправляется [METHOD] **/api/...**" + type: "frontend"
  2. Backend возвращает ответ: "Возвращается [STATUS] с {структура JSON}" + type: "backend"
  3. Frontend отображает результат: "Отображается [элемент UI] с [описание]" + type: "frontend"
  4. Integration действия (если есть): "Сравнивается/Проверяется/Создаётся/Обновляется [описание]" + type: "integration"
  
  🚨 КРИТИЧНО: Для каждого Scenario создавай codes для ВСЕХ реакций системы, описанных в требованиях:
  - Все API запросы и ответы
  - Все изменения UI
  - Все интеграционные действия
  - Все валидации и проверки
  - Все автоматические реакции системы

═══════════════════════════════════════════════════════════════
🚨 КРИТИЧЕСКИЕ ПРАВИЛА ДЛЯ CODE
═══════════════════════════════════════════════════════════════

0. 🚨 СТРОГО СЛЕДУЙ ТРЕБОВАНИЯМ! НЕ ВЫДУМЫВАЙ API, функциональность И НЕГАТИВНЫЕ СЦЕНАРИИ!
   ...
   🚨 КРИТИЧНО: НЕ ВЫДУМЫВАЙ НЕГАТИВНЫЕ СЦЕНАРИИ БЕЗ УПОМИНАНИЯ В ТРЕБОВАНИЯХ!
   
   ❌ ЗАПРЕЩЕНО создавать codes с ошибками (500, 400, 404, 504, таймауты), если они НЕ упомянуты явно в требованиях!
   ❌ ЗАПРЕЩЕНО создавать codes типа "Возвращается 500 Internal Server Error", "Возвращается 400 Bad Request", если это НЕ описано в требованиях!
   ❌ ЗАПРЕЩЕНО создавать codes типа "Возвращается 200 OK с {templateWithImage: []}" (пустой ответ), если в требованиях описан только успешный ответ с данными!
   ❌ ЗАПРЕЩЕНО добавлять негативные codes (ошибки, таймауты, пустые ответы) в позитивные scenarios!
   ❌ ЗАПРЕЩЕНО создавать отдельные scenarios для обработки ошибок, если они НЕ упомянуты в требованиях!
   
   ✅ ПРАВИЛЬНО: Если в требованиях описан только успешный ответ 200 OK с данными - создавай ТОЛЬКО его!
   ✅ ПРАВИЛЬНО: Если в требованиях НЕТ упоминания об обработке ошибок - НЕ создавай codes с ошибками!
   ✅ ПРАВИЛЬНО: Если в требованиях НЕТ упоминания о таймаутах - НЕ создавай codes с таймаутами!
   ✅ ПРАВИЛЬНО: Если в требованиях НЕТ упоминания о пустом ответе - НЕ создавай codes с пустым ответом!
   
   🎯 ПРАВИЛО: Model = ДОКУМЕНТАЦИЯ требований, НЕ твои догадки! Code = ТОЛЬКО то, что явно описано в требованиях!
...
2. ⚠️ КАЖДЫЙ Code должен быть ОТДЕЛЬНЫМ действием (НЕ объединяй frontend + backend + UI в один!)
   ...
   ✅ ПРАВИЛЬНО - РАЗДЕЛИТЬ на отдельные Codes:
   codes: [
     {text: "Отправляется GET **/api/v1/[resource]/{id}** с параметром id (type)", type: "frontend"},
     {text: "Возвращается 200 OK с {field1, field2, field3}", type: "backend"},
     {text: "Отображается детальная информация с полями field1, field2, field3", type: "frontend"},
     {text: "Создаётся запись в БД с параметрами field1, field2", type: "integration"}
   ]
   
   ❌ КРИТИЧЕСКАЯ ОШИБКА - ОБЪЕДИНЕНИЕ нескольких реакций:
   "POST /api/[resource] с param1, param2, Возвращается 400 Bad Request, Отобразить ошибку"
   ЭТО 3 ДЕЙСТВИЯ В ОДНОМ CODE! КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО!
   
   🎯 ПРАВИЛО: 1 Code = 1 тип действия (ТОЛЬКО запрос ИЛИ ТОЛЬКО ответ ИЛИ ТОЛЬКО UI ИЛИ ТОЛЬКО интеграция)
   
3. ⚠️ КРИТИЧНО: ОБЯЗАТЕЛЬНО генерируй backend коды для КАЖДОГО API-запроса!
   ...
   - Frontend: "Отправляется GET **/api/v1/[resource]**" → Backend: "Возвращается 200 OK с [{field1, field2}]"
   - Frontend: "Отправляется POST **/api/v1/[resource]** с параметрами..." → Backend: "Возвращается 200 OK с {field1, field2}" ИЛИ "Возвращается 400 Bad Request с {error: '...'}" (если описано в требованиях)
   - Frontend: "Отправляется DELETE **/api/v1/[resource]/{id}**" → Backend: "Возвращается 200 OK" ИЛИ "Возвращается 404 Not Found с {error: '...'}" (если описано в требованиях)
...
4. ⚠️ Code НЕ должен начинаться с "При..." (это условие, а не действие системы!)
   ❌ НЕПРАВИЛЬНО: "При выборе опции: выполнить действие X"
   ✅ ПРАВИЛЬНО: "Выполнить действие X с параметром Y"
   
   ❌ НЕПРАВИЛЬНО: "При нажатии кнопки X с выбранным параметром Y: POST /api/..."
   ✅ ПРАВИЛЬНО: "POST **/api/[resource]** с параметром param = value"

5. Code НЕ содержит префиксов "API:", "UI:", "Frontend:", "Backend:"
   ✅ "POST **/api/[resource]**"
...
═══════════════════════════════════════════════════════════════
📚 УНИВЕРСАЛЬНЫЕ ПРИМЕРЫ ПРАВИЛЬНОГО CODE
═══════════════════════════════════════════════════════════════

ПРИМЕР 1: Scenario с полной цепочкой Frontend → Backend → Frontend → Integration
Scenario: "Нажать на кнопку '[действие]'"
codes: [
  {"id": "uuid", "text": "Отправляется POST **/api/v1/[resource]** с параметрами param1 (type1), param2 (type2)", "type": "frontend"},
  {"id": "uuid", "text": "Возвращается 200 OK с {field1, field2, field3}", "type": "backend"},
  {"id": "uuid", "text": "Отображается модальное окно '[название]' с полями [описание]", "type": "frontend"},
  {"id": "uuid", "text": "Создаётся запись в БД с параметрами field1, field2", "type": "integration"}
]

ПРИМЕР 2: Scenario с цепочкой API запросов
Scenario: "Выполнить действие '[название]'"
codes: [
  {"id": "uuid", "text": "Отправляется GET **/api/v1/[resource1]**", "type": "frontend"},
  {"id": "uuid", "text": "Возвращается 200 OK с {field1, field2}", "type": "backend"},
  {"id": "uuid", "text": "Отправляется POST **/api/v1/[resource2]** с параметрами из ответа", "type": "frontend"},
  {"id": "uuid", "text": "Возвращается 200 OK с {result}", "type": "backend"},
  {"id": "uuid", "text": "Отображается страница '[название]' с данными result", "type": "frontend"}
]

ПРИМЕР 3: Scenario с условной логикой (разные ветки = разные Scenarios)
Scenario 1: "Открыть страницу при условии X"
codes: [
  {"id": "uuid", "text": "Отправляется GET **/api/v1/[resource]**", "type": "frontend"},
  {"id": "uuid", "text": "Возвращается 200 OK с {condition: true, data: {...}}", "type": "backend"},
  {"id": "uuid", "text": "Отображается страница '[вариант A]'", "type": "frontend"}
]

Scenario 2: "Открыть страницу при условии Y"
codes: [
  {"id": "uuid", "text": "Отправляется GET **/api/v1/[resource]**", "type": "frontend"},
  {"id": "uuid", "text": "Возвращается 200 OK с {condition: false, data: {...}}", "type": "backend"},
  {"id": "uuid", "text": "Отображается страница '[вариант B]'", "type": "frontend"}
]

═══════════════════════════════════════════════════════════════
❌ УНИВЕРСАЛЬНЫЕ ПРИМЕРЫ НЕПРАВИЛЬНОГО CODE
═══════════════════════════════════════════════════════════════
...
✅ "Отображается модальное окно '[название]'"

ОШИБКА 3: Наличие поля requirement
❌ {"text": "GET **/api/[resource]**", "requirement": "2.2.1"}
✅ {"text": "GET **/api/[resource]**"}

ОШИБКА 4: Scenario с нумерацией
❌ "1. Нажать на кнопку '[название]'"
✅ "Нажать на кнопку '[название]'"

ОШИБКА 5: Объединение нескольких реакций в один Code
❌ "GET **/api/[resource]**, Возвращается 200 OK с {...}, Отобразить список"
✅ Разделить на 3 отдельных Code (frontend, backend, frontend)

═══════════════════════════════════════════════════════════════
🔍 УНИВЕРСАЛЬНЫЙ АЛГОРИТМ ВЫЯВЛЕНИЯ ВСЕХ ВЕТВЛЕНИЙ
═══════════════════════════════════════════════════════════════

🚨 КРИТИЧНО: НЕ ограничивайся только явно заданными шагами, условиями или случаями — выявляй и описывай implicit (неявные) ветвления, альтернативные и возвратные флоу.

1. АНАЛИЗ ЯВНЫХ ДЕЙСТВИЙ:
   - Извлеки все явно описанные действия пользователя
   - Создай Scenarios для каждого явного действия
   - Для каждого Scenario определи полную цепочку codes

2. АНАЛИЗ НЕЯВНЫХ (IMPLICIT) ТРИГГЕРОВ:
   - Найди все упоминания автоматических реакций системы
   - Определи, какое пользовательское действие (явное или неявное) их триггерит
   - Создай Scenarios для каждого implicit триггера
   - Примеры: автозаполнение при загрузке, фильтрация при изменении, валидация при потере фокуса, автоматическое обновление UI, блокировка/разблокировка элементов

3. АНАЛИЗ ВСЕХ ВОЗМОЖНЫХ ВАРИАНТОВ:
   - Для каждого действия анализируй ВСЕ возможные варианты:
     * Успешное выполнение
     * Неуспешное выполнение (если описано в требованиях)
     * Отмена действия
     * Отсутствие данных
     * Невалидные параметры (если описана валидация)
     * Альтернативные действия
     * Возврат к предыдущему состоянию
     * Корректировка введённых данных
     * Автоматические UI-события
   - Каждый вариант = отдельный Scenario, если результат отличается

4. АНАЛИЗ УСЛОВНОЙ ЛОГИКИ:
   - Найди все условия ("если", "при", "когда", "в случае", "в зависимости от")
   - Для каждого условия создай отдельный Scenario для каждой ветки
   - Пример: "Если пользователь авторизован → показать X, иначе → показать Y"
     → Scenario 1: "Открыть страницу будучи авторизованным"
     → Scenario 2: "Открыть страницу будучи неавторизованным"

5. АНАЛИЗ АЛЬТЕРНАТИВНЫХ ФЛОУ:
   - Найди все альтернативные пути выполнения задачи
   - Каждый альтернативный путь = отдельный Scenario
   - Примеры: разные способы оплаты, разные методы входа, разные варианты заполнения формы

6. АНАЛИЗ ВОЗВРАТНЫХ СЦЕНАРИЕВ:
   - Найди все возможности вернуться, отменить, изменить выбор
   - Каждая возможность = отдельный Scenario
   - Примеры: "Отменить заполнение формы", "Вернуться к предыдущему шагу", "Изменить выбранное значение"

7. АНАЛИЗ ВСЕХ СИСТЕМНЫХ РЕАКЦИЙ:
   - Для каждого Scenario определи ВСЕ реакции системы:
     * Все API запросы и ответы
     * Все изменения UI
     * Все интеграционные действия
     * Все валидации и проверки
     * Все автоматические реакции системы
   - Создай codes для каждой реакции (frontend запрос, backend ответ, frontend отображение, integration действия)

8. ПРОВЕРКА ПОЛНОТЫ ПОКРЫТИЯ:
   - Убедись, что все описанные в требованиях варианты покрыты Scenarios
   - Убедись, что для каждого Scenario есть полная цепочка codes
   - Убедись, что все implicit ветвления выявлены
   - Убедись, что все альтернативные флоу покрыты

🎯 РЕЗУЛЬТАТ: Структурированная тестовая модель, пригодная для генерации как пользовательских, так и интеграционных автоматизированных тест-кейсов любого типа.
...
🚨 КРИТИЧНО: НЕ ВЫДУМЫВАЙ НЕГАТИВНЫЕ СЦЕНАРИИ БЕЗ УПОМИНАНИЯ В ТРЕБОВАНИЯХ!
❌ НЕ создавай codes с ошибками (500, 400, 404, 504, таймауты), если они НЕ упомянуты в требованиях!
❌ НЕ создавай codes с пустыми ответами, если это НЕ описано в требованиях!
❌ НЕ добавляй негативные codes в позитивные scenarios!
❌ НЕ создавай отдельные scenarios для обработки ошибок, если они НЕ упомянуты в требованиях!

✅ ПРАВИЛЬНО: Story описывает бизнес-ценность для пользователя (НЕ технические процессы типа "Загрузка страницы", "Проверка подключения")
...
- Всегда анализируй требования подробно, выделяй все возможные действия пользователя (как явные, так и неявные/implicit).
- Не ограничивайся только прямыми глаголами — если бизнес-логика, UX или описание потока подразумевают пользовательское воздействие, выделяй это как Scenario.
- Для каждого варианта пользовательского действия (“если вариант X”, “при вводе Y”, “при ошибке”) добавляй Scenario и соответствующий Code.
- Чем проще структура модели — тем лучше. Не дроби Story ради технических деталей, но обязательно покрывай все пользовательские ветвления внутри Story.
`.trim();
...
1. СТРУКТУРА
   ...
   - Story = USER STORY (потребность пользователя с бизнес-правилами, НЕ технические процессы!)
     ❌ НЕПРАВИЛЬНО: "Загрузка страницы", "Проверка подключения", "Обработка ошибок"
     ✅ ПРАВИЛЬНО: "Создание QR-кода", "Регистрация пользователя", "Просмотр каталога"
            ...
            for (let attempt = 0; attempt < MAX_MODEL_ATTEMPTS_PER_CHUNK; attempt++) {
                ...
                try {
                    ...
                    const interactiveResult = await runInteractiveLLM({
                        ...
                        maxIterations: 10,  // ✅ Уменьшено с 20 до 10 для предотвращения зацикливания и превышения лимита токенов
                        modelOptions: {
                            temperature: 0,
                            top_p: 0.9,
...
async function fixTestCasesAsync({ taskId, testCases, fixPrompt, projectId, bearerToken, requirements }) {
    ...
    // ✅ ИСПОЛЬЗУЕМ УПРОЩЁННЫЙ ПРОМПТ ДЛЯ ФОРМАТЕРА (только стайл-гайд, без правил покрытия)
    // Форматер получает уже готовые тест-кейсы и только приводит их к идеальному формату
    const FORMATTER_SYSTEM_PROMPT = `
Ты — Форматер тест-кейсов. Твоя задача — привести уже готовые тест-кейсы к идеальному формату.

🚨 КРИТИЧНО: НЕ переизобретай структуру тест-кейса, только поправь формат по правилам ниже!

📝 ПРАВИЛА ФОРМАТА:

**TITLE:**
- БЕЗ "проверка|тестирование|валидация"
- От лица пользователя для E2E: "Создать платеж" (НЕ "Проверка создания платежа")

**STEPS:**
❌ ЗАПРЕЩЕНО: "Попытаться", "Дождаться", "Проверить", "Убедиться"
❌ ЗАПРЕЩЕНО: Описания состояний: "открытия модального окна", "загрузки списка"
✅ ТОЛЬКО действия: "Нажать кнопку", "Ввести текст", "Выбрать файл"

**PRECONDITION:**
- E2E Tests: обычно "" (пустая строка)
- Integration frontend Tests: ОБЯЗАТЕЛЬНО! Описание состояния UI (БЕЗ действий пользователя!)
  ✅ Правильно: "Пользователь авторизован, на странице создания QR-кода"
  ❌ Неправильно: "Нажать кнопку, ввести данные" (это steps!)
- Integration backend Tests: ОБЯЗАТЕЛЬНО! Описание состояния сервера

**EXPECTED:**
✅ **Отображается** (UI), **Возвращается** (API), **Скрывается** (UI), **Создан/Удалён/Обновлён** (объекты)
❌ **Обновляется**, **Удаляется** — НЕПРАВИЛЬНО!
✅ Всегда начинается с **жирного** ключевого слова!

**INTEGRATION FRONTEND - ОСОБЫЕ ПРАВИЛА:**
🚨 КРИТИЧНО: steps = ТОЛЬКО пользовательские действия (Нажать, Ввести, Выбрать)
🚨 КРИТИЧНО: Технические детали (API запросы) → в precondition или expected, НЕ в steps!
✅ Правильно в steps: "Нажать кнопку 'Создать QR-код'"
...
📊 ПАРАМЕТРИЗАЦИЯ:
✅ Используй parameters + examples для вариативности (категории, форматы, размеры, места размещения кнопок)
✅ Если кнопка/элемент в разных местах, но реакция системы одинаковая → ОДИН тест с параметризацией!
❌ НЕ создавай дубликаты для разных значений — параметризуй!

🚨 КРИТИЧНО: НЕ ВЫДУМЫВАЙ ИНФОРМАЦИЮ, КОТОРОЙ НЕТ В ТРЕБОВАНИЯХ!
✅ Используй ТОЛЬКО то, что явно указано в requirements!
