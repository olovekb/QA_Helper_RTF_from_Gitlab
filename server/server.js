...
- Story = пользовательская цель с бизнес-правилами (НЕ контролы, НЕ "Загрузка...").
- Scenario = одно пользовательское действие/implicit trigger. Текст начинается с глагола.
- Code = полная реакция системы на действие. Каждая реакция отдельным объектом, строго type: frontend | backend | integration.

🔄 АЛГОРИТМ ACTION → REACTION
1. Найди все явные и неявные действия пользователя, ветви и условия.
2. Для каждого действия создай Scenario.
3. Для Scenario распиши цепочку Codes: фронт → бэк → фронт → интеграции, пока реакция не завершена.
4. Не смешивай действия и реакции. Не используй placeholder-формулировки.
        ...
        "scenarios": [
          {
            ...
            "codes": [
              { "id": "uuid", "text": "Реакция системы", "type": "frontend|backend|integration" }
            ]
          }
        ]
      }
