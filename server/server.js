...
- Scenario = одно пользовательское действие (НЕ техническое!). Текст начинается с глагола действия пользователя: "Нажать", "Ввести", "Выбрать". ❌ НЕ "Загрузить страницу", "Открыть приложение" - это технические действия!
...
1. Найди все явные и неявные действия пользователя (НЕ технические действия типа "Загрузить страницу"!).
2. Для каждого действия пользователя создай Scenario (глагол действия: "Нажать", "Ввести", "Выбрать").
3. Для Scenario распиши цепочку Codes: frontend (отправка запросов) → backend (возврат ответов) → frontend (отображение UI), пока реакция не завершена.
...
- Не создавай Story по разделам документа! Story = бизнес-ценность для пользователя, определяй по смыслу, а НЕ по структуре документа.
- Не создавай Story/Scenario ради оглавления или технического раздела. Есть ценность → есть Story; нет ценности → пропусти.
- Scenario = действие пользователя (НЕ техническое!). ❌ "Загрузить страницу", "Открыть приложение" - это НЕ действия пользователя! ✅ "Нажать кнопку", "Ввести текст" - это действия пользователя.
