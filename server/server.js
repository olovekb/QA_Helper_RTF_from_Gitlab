...
ТВОЯ ЗАДАЧА: Построить структурированную модель, которая покрывает ВСЕ возможные пользовательские пути и системные реакции, включая implicit (неявные) ветвления, альтернативные флоу и возвратные сценарии.

═══════════════════════════════════════════════════════════════
📖 ПРАВИЛА ДЛЯ SCENARIOS (ПОЛЬЗОВАТЕЛЬСКИЕ ДЕЙСТВИЯ)
═══════════════════════════════════════════════════════════════

🚨 КРИТИЧНО: Для КАЖДОЙ Story выявляй и создавай отдельные Scenarios НЕ ТОЛЬКО на основании явно сформулированных действий, но и на основе ВСЕХ возможных логических и бизнес-вариантов поведения пользователя, которые подразумеваются логикой требований.

1. ОСНОВНОЙ ПРИНЦИП: Дроби модель так, чтобы каждый осмысленный пользовательский путь (любое ветвление в UX или данных) всегда становился отдельным Scenario — даже если он не прописан явно как шаг в исходном тексте.

2. ВЫЯВЛЕНИЕ ЯВНЫХ ДЕЙСТВИЙ:
   - Извлекай все явно описанные действия пользователя (клики, вводы, выборы, переходы)
   - Каждое атомарное действие = отдельный Scenario
   - Scenario начинается с глагола действия: "Нажать", "Ввести", "Выбрать", "Кликнуть", "Открыть", "Перейти", "Заполнить", "Загрузить", "Применить", "Подтвердить", "Закрыть", "Сохранить", "Отменить"

3. ВЫЯВЛЕНИЕ НЕЯВНЫХ (IMPLICIT) ДЕЙСТВИЙ И ВЕТВЛЕНИЙ:
   ...
   - Примеры implicit триггеров:
     * Автозаполнение полей при загрузке формы → Scenario: "Загрузить форму с предзаполненными данными"
     * Фильтрация списка при изменении параметров → Scenario: "Изменить параметр фильтрации"
     * Валидация при потере фокуса → Scenario: "Убрать фокус с поля ввода"
     * Автоматическое обновление UI при изменении данных → Scenario: "Дождаться обновления данных"
     * Блокировка/разблокировка элементов при изменении условий → Scenario: "Изменить условие, влияющее на доступность"

4. ВЫЯВЛЕНИЕ ВСЕХ ВОЗМОЖНЫХ ВАРИАНТОВ И ВЕТВЛЕНИЙ:
   - Для каждого действия анализируй ВСЕ возможные варианты:
     ...
     * Автоматические UI-события (автозаполнение, автоподстановка, автопереключение)
   
   - Каждый вариант = отдельный Scenario, если результат отличается

5. РАЗБИВКА УСЛОВНОЙ ЛОГИКИ:
   - Если в требованиях есть условия ("если...", "при...", "когда...", "в случае...") — создавай отдельный Scenario для каждой ветки
...
6. ВЫЯВЛЕНИЕ АЛЬТЕРНАТИВНЫХ ФЛОУ:
   - Анализируй требования на предмет альтернативных путей выполнения задачи
...
7. ВЫЯВЛЕНИЕ ВОЗВРАТНЫХ СЦЕНАРИЕВ:
   - Если пользователь может вернуться, отменить, изменить выбор — создавай отдельные Scenarios
...
8. ЗАПРЕЩЁННЫЕ ФОРМУЛИРОВКИ:
   ...
   - ❌ "Дождаться" — это лишнее ожидание, пропусти его
   - ❌ "Попытаться" — просто опиши действие без "попытаться"
   - ❌ Служебные названия: "Выполнить пользовательское действие", "Обработать сценарий", "Сценарий 1"
   - ✅ Сценарий ВСЕГДА описывает конкретное действие из требований: "Авторизоваться в системе", "Выбрать тип 'Многоразовый QR-код'", "Нажать кнопку 'Получить QR-код'"

═══════════════════════════════════════════════════════════════
⚙️ ПРАВИЛА ДЛЯ CODES (РЕАКЦИИ СИСТЕМЫ)
═══════════════════════════════════════════════════════════════

🚨 КРИТИЧНО: Для каждого Scenario полноценно и отдельно формируй массив codes с реакциями системы. Любые изменения UI, интеграционные действия, сравнения данных, проверки API, бизнесовые условия, алерты и реакции системы фиксируй как отдельные Codes.

1. ОСНОВНОЙ ПРИНЦИП: Code описывает ОДНУ реакцию системы. НЕ объединяй несколько реакций в один Code!

2. ТИПЫ CODES И ИХ ФОРМАТИРОВАНИЕ:

   **Frontend (type: "frontend")** — UI-реакции и отправка запросов:
   - Отображение элементов: "Отображается модальное окно 'Подтверждение'"
   - Скрытие элементов: "Скрывается форма ввода"
   - Заполнение данных: "Заполняется список из параметра X"
   - Валидация: "Отображается ошибка 'Поле обязательно для заполнения'"
   - Блокировка/разблокировка: "Поле становится недоступным для редактирования"
   - Отправка запросов: "Отправляется GET **/api/v1/users**"
   - Отправка с параметрами: "Отправляется POST **/api/v1/orders** с параметрами productId (number), quantity (number)"

   **Backend (type: "backend")** — ответы API:
   - Успешные ответы: "Возвращается 200 OK с {id, name, email}"
   - Ответы с массивами: "Возвращается 200 OK с массивом: [{id, name, price}]"
   - Ошибочные ответы (только если описаны в требованиях): "Возвращается 400 Bad Request с {error: 'Invalid data'}"
   - Всегда указывай структуру JSON в ответе!

   **Integration (type: "integration")** — интеграционные действия и проверки:
   - Сравнения данных: "Сравнивается значение X с Y"
   - Проверки условий: "Проверяется условие: X равен Y"
   - Изменения в БД/кэше: "Создаётся запись в БД с параметрами X, Y"
   - Интеграции с внешними системами: "Отправляется запрос во внешнюю систему Z"

3. ЦЕПОЧКА РЕАКЦИЙ ДЛЯ ОДНОГО SCENARIO:
   Для каждого Scenario формируй ПОЛНУЮ цепочку реакций системы:
   
   Пример для Scenario "Нажать кнопку 'Создать заказ'":
   codes: [
     {text: "Отправляется POST **/api/v1/orders** с параметрами productId (number), quantity (number)", type: "frontend"},
     {text: "Возвращается 200 OK с {orderId, orderNumber, total, status}", type: "backend"},
     {text: "Отображается экран 'Заказ создан' с данными заказа", type: "frontend"},
     {text: "Создаётся запись в БД с orderId, productId, quantity, total", type: "integration"}
   ]

4. ПОКРЫТИЕ ВСЕХ ВЕТВЛЕНИЙ В CODES:
   - Для каждого варианта Scenario (успех, неуспех, отмена, отсутствие данных, невалидные параметры) создавай соответствующие codes
   - Если в требованиях описана обработка ошибок — создавай codes с ошибками
   - Если в требованиях описана валидация — создавай codes с валидационными сообщениями
   - Если в требованиях описаны альтернативные реакции — создавай codes для каждой альтернативы

5. ЗАПРЕЩЁННЫЕ ФОРМУЛИРОВКИ:
   - ❌ "Обработать действие:", "Реализовать логику:", "Выполнить обработку:" — это внутренняя логика, НЕ code!
   - ❌ Префиксы "API:", "UI:", "Frontend:", "Backend:" — НЕ используй!
   - ❌ Объединение нескольких реакций в один code — КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО!

6. КРИТИЧНО: НЕ ВЫДУМЫВАЙ НЕГАТИВНЫЕ РЕАКЦИИ!
   - ❌ ЗАПРЕЩЕНО создавать codes с ошибками (500, 400, 404, таймауты), если они НЕ упомянуты в требованиях!
   - ❌ ЗАПРЕЩЕНО создавать codes с "пустой ответ", "ошибка сервера", если это НЕ описано в требованиях!
   - ✅ ПРАВИЛЬНО: Если в требованиях описан только успешный ответ 200 OK — создавай ТОЛЬКО его!
   - ✅ ПРАВИЛЬНО: Если в требованиях НЕТ упоминания об обработке ошибок — НЕ создавай codes с ошибками!
   - 🎯 ПРАВИЛО: Code = ТОЛЬКО то, что явно описано в требованиях или логически следует из них!

═══════════════════════════════════════════════════════════════
🔍 АЛГОРИТМ АНАЛИЗА ТРЕБОВАНИЙ
═══════════════════════════════════════════════════════════════

1. ПЕРВЫЙ ПРОХОД: Выявление явных действий
   - Найди все глаголы действий пользователя
   - Создай Scenarios для каждого явного действия

2. ВТОРОЙ ПРОХОД: Выявление implicit триггеров
...
3. ТРЕТИЙ ПРОХОД: Выявление всех ветвлений
   - Найди все условия ("если", "при", "когда", "в случае")
   - Для каждого условия создай отдельные Scenarios для каждой ветки
   - Найди все альтернативные пути выполнения
   - Создай Scenarios для каждого альтернативного пути

4. ЧЕТВЁРТЫЙ ПРОХОД: Выявление всех системных реакций
   - Для каждого Scenario определи все реакции системы
   - Создай codes для каждой реакции (frontend запрос, backend ответ, frontend отображение, интеграционные действия)

5. ПЯТЫЙ ПРОХОД: Проверка полноты покрытия
   ...
   - 🚨 Каждая Story из заданной структуры ДОЛЖНА содержать ≥1 Scenario. Пустые Stories запрещены!
