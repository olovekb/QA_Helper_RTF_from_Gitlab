...
1. Story = бизнес-ценность для пользователя (НЕ техническая реализация!)
2. Scenario = атомарное действие пользователя (глагол: "Нажать", "Ввести", "Выбрать")
...
ТИПЫ:
- **frontend**: "Отправляется GET **/api/...**", "Отображается модальное окно", "Отображается ошибка"
- **backend**: "Возвращается 200 OK с {id, name}", "Возвращается 400 Bad Request с {error: '...'}" (только если описано!)
- **integration**: "Сравнивается X с Y", "Создаётся запись в БД"
...
1. Найди все явные действия пользователя → создай Scenarios
...
📊 ПАРАМЕТРИЗАЦИЯ:
✅ Используй parameters + examples для вариативности (категории, форматы, размеры)
