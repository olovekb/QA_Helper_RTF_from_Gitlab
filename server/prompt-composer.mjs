...
1. **Граничные значения (Boundary Values):**
   - Если есть диапазон (сумма, длина), создай ОДИН тест с параметризацией для: Min-1, Min, Max, Max+1.
   - Пример: Лимит 1 млн. ОДИН тест с параметром {{сумма}} и examples: [{сумма: "999999.99"}, {сумма: "1000000"}, {сумма: "1000000.01"}]
   - ❌ НЕ создавай 4 отдельных теста! Используй parameters + examples!
...
5. **Негативные сценарии (ОБЯЗАТЕЛЬНО!):**
   - Для каждого поля с валидацией создай ОДИН Integration frontend тест с параметризацией (пустое, спецсимволы, невалидный формат).
   - Для каждого API создай ОДИН Integration backend тест с параметризацией на ошибки (400, 500, таймаут).
   - Для каждой логики "Если... то... иначе..." создай тест на ветку "Иначе".
   - ❌ НЕ создавай отдельный тест для каждого негативного случая! Используй parameters + examples!
...
🎯 ПРИОРИТЕТЫ И ЛИМИТЫ:
   ...
   - Для форм с валидацией: ОБЯЗАТЕЛЬНО тесты на нарушение валидации (через параметризацию!)
   - Для UI логики (скрытие/очистка): ОБЯЗАТЕЛЬНО тесты на проверку этой логики
   
🚨 КРИТИЧНО: Применяй техники тест-дизайна ЧЕРЕЗ ПАРАМЕТРИЗАЦИЮ, а не через создание множества отдельных тестов!
   - Граничные значения (Min-1, Min, Max, Max+1) → ОДИН тест с 4 examples
   - Классы эквивалентности (пустое, спецсимволы, невалидный формат) → ОДИН тест с 3-4 examples
   - Негативные сценарии (400, 500, таймаут) → ОДИН тест с 3 examples
   
   ⚠️ ПРИОРИТЕТЫ ПРИ ПРЕВЫШЕНИИ ЛИМИТА:
   1. Позитивные тесты для каждого Code (ОБЯЗАТЕЛЬНО!)
   2. Негативные тесты с параметризацией (объединяй через examples!)
   3. Граничные значения (объединяй в ОДИН тест!)
   4. UI логика и зависимости (если осталось место)
   
   ❌ НИКОГДА не превышай лимит 8-12 Integration тестов на Scenario!
   ✅ Если нужно покрыть больше - используй параметризацию для объединения!
`,
