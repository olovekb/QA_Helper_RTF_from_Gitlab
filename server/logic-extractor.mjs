...
1. **Валидация полей:** Мин/макс длина, запрещенные символы, форматы, обязательность.

2. **Числовые границы:** Мин/макс суммы, диапазоны дат, лимиты.

3. **Условная логика UI:** "Если пришло X, то отобразить Y, иначе Z".

4. **Состояния ошибок:** Что происходит при 500, 400, таймауте или пустом ответе.

5. **Зависимости полей:** Если выбран чекбокс А, то поле Б становится обязательным.

6. **Автозаполнение:** Когда поля заполняются автоматически.

7. **Очистка полей:** Когда поля очищаются при изменении состояния.
...
`);
    ...
    if (logicConstraints.negative_scenarios && logicConstraints.negative_scenarios.length > 0) {
        parts.push(`
═══════════════════════════════════════════════════════════════
❌ НЕГАТИВНЫЕ СЦЕНАРИИ (ОБЯЗАТЕЛЬНО ПРОТЕСТИРОВАТЬ!)
═══════════════════════════════════════════════════════════════
