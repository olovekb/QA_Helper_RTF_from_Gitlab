...
async function getJobId(projectId) {
    try {
        const jobSuggestUrl = `https://abanking.qatools.cloud/api/job/suggest?projectId=${projectId}&size=20`;
...
async function setJobsMapping(projectId, treeId, jobId) {
    try {
        const statsUrl = 'https://abanking.qatools.cloud/api/v2/test-case/bulk/job/stats';
        const requestBody = {
            selection: {
                inverted: true,
                groupsInclude: [],
...
// ... (остальной код остаётся без изменений до createTestPlan)
...
// ... (остальные функции getTreeId, getJobId, setJobsMapping остаются без изменений)